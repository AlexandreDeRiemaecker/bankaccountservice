...
describe('BankAccountsService', () => {
  ...
  it('should throw an error if bank account already exists', async () => {
    ...
    await expect(service.create(createBankAccountDto)).rejects.toThrow(
      new ConflictException('BankAccount with this IBAN already exists'),
    );
  });

  it('should throw an error if person is not found when creating a bank account', async () => {
    const createBankAccountDto: CreateBankAccountDto = {
      personId: '1',
      IBAN: 'DE1234567890',
      currentBalance: 1000,
    ...
    jest
      .spyOn(neptuneService, 'findVertexByProperty')
      .mockResolvedValueOnce(null) // for checking existing account
      .mockResolvedValueOnce(null); // for checking person existence
  ...
  it('should throw an error if bank account is not found', async () => {
    ...
    await expect(service.findOne('DE1234567890')).rejects.toThrow(
      new NotFoundException('BankAccount not found'),
    );
  });
  ...
  it('should throw an error if bank account to delete is not found', async () => {
    jest
      .spyOn(neptuneService, 'deleteVertex')
      .mockRejectedValue(new NotFoundException('BankAccount not found'));

    await expect(service.remove('DE1234567890')).rejects.toThrow(
      new NotFoundException('BankAccount not found'),
    );
  });
