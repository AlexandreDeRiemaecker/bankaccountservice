...
describe('BankTransactionsService', () => {
  ...
  it('should find all bank transactions', async () => {
    const bankTransactions: BankTransactionDto[] = [
      {
  ...
  it('should find one bank transaction', async () => {
    const bankTransaction: BankTransactionDto = {
      transactionId: 'transactionId',
      destinationIBAN: 'DE9876543210',
      amount: 1000,
    };
  ...
  it('should throw an error if bank transaction is not found', async () => {
    jest.spyOn(neptuneService, 'findVertexByProperty').mockResolvedValue('');
  ...
  it('should throw an error if bank transaction to remove is not found', async () => {
    jest.spyOn(neptuneService, 'deleteVertex').mockResolvedValue('');

    await expect(service.remove('transactionId')).rejects.toThrow(
      'BankTransaction not found',
    );
  });
